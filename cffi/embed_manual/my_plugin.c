...
// def do_stuff(p):
100,101,102,32,100,111,95,115,116,117,102,102,40,112,41,58,10,
//     print("adding %d and %d" % (p.x, p.y))
32,32,32,32,112,114,105,110,116,40,34,97,100,100,105,110,103,32,37,100,32,97,
110,100,32,37,100,34,32,37,32,40,112,46,120,44,32,112,46,121,41,41,10,
//     return p.x + p.y
32,32,32,32,114,101,116,117,114,110,32,112,46,120,32,43,32,112,46,121,10,
0 };
#ifdef PYPY_VERSION
...
static void _cffi_checkfld_typedef_point_t(point_t *p)
{
  ...
  (void)p;
  (void)((p->x) | 0);  /* check that 'point_t.x' is an integer */
  (void)((p->y) | 0);  /* check that 'point_t.y' is an integer */
}
struct _cffi_align_typedef_point_t { char x; point_t y; };

static struct _cffi_externpy_s _cffi_externpy__do_stuff =
  { "my_plugin.do_stuff", (int)sizeof(int), 0, 0 };
